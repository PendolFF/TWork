class Program
{
    ...
    // Метод, который фильтрует массив строк по длине
    static string[] FilterArrayByLength(string[] array, int maxLength)
    {
        // Подсчитывается количество строк в массиве, удовлетворяющих условию (длина меньше или равна заданной)
        int count = 0;
        ...
        // Создается новый массив подходящей длины
        string[] result = new string[count];
        int index = 0;
        // Заполняется новый массив строками, удовлетворяющими условию
        foreach (string str in array)
        {
            if (str.Length <= maxLength)
            {
